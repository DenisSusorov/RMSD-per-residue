# RMSD-per-residue
This bash script extracts matching atoms in two PDBs, calculates per residue RMSD for them and writes a new PDB with coordinates from the first PDB and with per atom RMSD in the b-factor column.  
**Requirements**:
...
1. I aligned two models of ribosome by a chain in the large subunit to find the relative orientation of small subunits. I set b-factors to 0 (setattr atoms bfactor 0) and saved both small subunits as separate PDB files. 
...
2. I run the script and got [NAME of the first PDB]-RMSD.pdb. Here, the PDB is colored by b-factor (in ChimeraX toolbar, molecule display->b-factor), which in this case represents coloring by RMSD.
